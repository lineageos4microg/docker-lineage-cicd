...
for branch in ${BRANCH_NAME//,/ }; do
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    if [ "$LOCAL_MIRROR" = true ]; then
      ( yes||: ) | repo init -u https://github.com/LineageOS-UL/android.git --reference "$MIRROR_DIR" -b "$branch" --git-lfs &>> "$repo_log"
    else
      ( yes||: ) | repo init -u https://github.com/LineageOS-UL/android.git -b "$branch" --git-lfs &>> "$repo_log"
    fi
