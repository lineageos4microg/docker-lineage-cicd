    ...
    for codename in ${devices//,/ }; do
      if ! [ -z "$codename" ]; then
        ...
        if brunch $codename &>> "$DEBUG_LOG"; then
          ...
          [ -f recovery.img ] && recovery_source_file='recovery.img' || recovery_source_file='boot.img'
          find . -maxdepth 1 -name $recovery_source_file -type f -exec cp {} "$ZIP_DIR/$zipsubdir/${recovery_name}" \; &>> "$DEBUG_LOG"
          cd "$source_dir"
